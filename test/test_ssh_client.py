...
class TestExecute(unittest.TestCase):
    ...
    def test_001_execute_async(self, client, policy, logger):
        ...
        self.assertIn(mock.call.log(level=logging.DEBUG, msg=command_log), log.mock_calls)
    ...
    def test_003_execute_async_no_stdout_stderr(self, client, policy, logger):
        ...
        # noinspection PyTypeChecker
        result = ssh.execute_async(command=command, open_stdout=False)

        self.assertIn(chan, result)
        chan.assert_has_calls(
            (mock.call.makefile("wb"), mock.call.makefile_stderr("rb"), mock.call.exec_command("{}\n".format(command)))
        )

        chan.reset_mock()
        result = ssh.execute_async(command=command, open_stderr=False)

        self.assertIn(chan, result)
        chan.assert_has_calls(
            (mock.call.makefile("wb"), mock.call.makefile("rb"), mock.call.exec_command("{}\n".format(command)))
        )

        chan.reset_mock()
        result = ssh.execute_async(command=command, open_stdout=False, open_stderr=False)

        self.assertIn(chan, result)
        chan.assert_has_calls((mock.call.makefile("wb"), mock.call.exec_command("{}\n".format(command))))
    ...
    def test_009_execute_async_verbose(self, client, policy, logger):
        ...
        log = logger(ssh.__class__.__name__).getChild("{host}:{port}".format(host=host, port=port))
        self.assertIn(mock.call.log(level=logging.INFO, msg=command_log), log.mock_calls)
    ...
    def test_010_execute_async_mask_command(self, client, policy, logger):
        ...
        log = logger(ssh.__class__.__name__).getChild("{host}:{port}".format(host=host, port=port))
        self.assertIn(mock.call.log(level=logging.DEBUG, msg=cmd_log), log.mock_calls)
    ...
    def test_011_check_stdin_str(self, client, policy, logger):
        ...
        get_transport.assert_called_once()
        open_session.assert_called_once()
        stdin.assert_has_calls([mock.call.write("{val}\n".format(val=stdin_val)), mock.call.flush()])
    ...
    @staticmethod
    def get_patched_execute_async_retval(ec=0, stderr_val=None, open_stdout=True, open_stderr=True, cmd_log=None):
        ...
        exp_result = exec_result.ExecResult(
            cmd=cmd_log if cmd_log is not None else command, stderr=err, stdout=out, exit_code=ec
        )

        return chan, "", exp_result, stderr, stdout

    @mock.patch("exec_helpers.ssh_client.SSHClient.execute_async")
    def test_019_execute(self, execute_async, client, policy, logger):
        (chan, _stdin, exp_result, stderr, stdout) = self.get_patched_execute_async_retval()
        ...
        log.assert_has_calls(
            [mock.call(level=logging.DEBUG, msg=str(x.rstrip().decode("utf-8"))) for x in stdout_list]
            + [mock.call(level=logging.DEBUG, msg=str(x.rstrip().decode("utf-8"))) for x in stderr_list]
            + [mock.call(level=logging.DEBUG, msg=message)]
        )

    @mock.patch("exec_helpers.ssh_client.SSHClient.execute_async")
    def test_020_execute_verbose(self, execute_async, client, policy, logger):
        (chan, _stdin, exp_result, stderr, stdout) = self.get_patched_execute_async_retval()
        is_set = mock.Mock(return_value=True)
        chan.status_event.attach_mock(is_set, "is_set")
        ...
        log = logger(ssh.__class__.__name__).getChild("{host}:{port}".format(host=host, port=port)).log
        log.assert_has_calls(
            [mock.call(level=logging.INFO, msg=str(x.rstrip().decode("utf-8"))) for x in stdout_list]
            + [mock.call(level=logging.INFO, msg=str(x.rstrip().decode("utf-8"))) for x in stderr_list]
            + [mock.call(level=logging.INFO, msg=message)]
        )

    @mock.patch("exec_helpers.ssh_client.SSHClient.execute_async")
    def test_021_execute_no_stdout(self, execute_async, client, policy, logger):
        (chan, _stdin, exp_result, stderr, stdout) = self.get_patched_execute_async_retval(open_stdout=False)
        ...
        # noinspection PyTypeChecker
        result = ssh.execute(command=command, verbose=False, open_stdout=False)

        self.assertEqual(result, exp_result)
        execute_async.assert_called_once_with(command, verbose=False, open_stdout=False)
        message = self.gen_cmd_result_log_message(result)
        log = logger(ssh.__class__.__name__).getChild("{host}:{port}".format(host=host, port=port)).log
        log.assert_has_calls(
            [mock.call(level=logging.DEBUG, msg=str(x.rstrip().decode("utf-8"))) for x in stderr_list]
    ...
    @mock.patch("exec_helpers.ssh_client.SSHClient.execute_async")
    def test_022_execute_no_stderr(self, execute_async, client, policy, logger):
        (chan, _stdin, exp_result, stderr, stdout) = self.get_patched_execute_async_retval(open_stderr=False)
        chan.status_event.attach_mock(mock.Mock(return_value=True), "is_set")
        ...
        # noinspection PyTypeChecker
        result = ssh.execute(command=command, verbose=False, open_stderr=False)

        self.assertEqual(result, exp_result)
        execute_async.assert_called_once_with(command, verbose=False, open_stderr=False)
        message = self.gen_cmd_result_log_message(result)
        log = logger(ssh.__class__.__name__).getChild("{host}:{port}".format(host=host, port=port)).log
        log.assert_has_calls(
            [mock.call(level=logging.DEBUG, msg=str(x.rstrip().decode("utf-8"))) for x in stdout_list]
    ...
    @mock.patch("exec_helpers.ssh_client.SSHClient.execute_async")
    def test_023_execute_no_stdout_stderr(self, execute_async, client, policy, logger):
        (chan, _stdin, exp_result, stderr, stdout) = self.get_patched_execute_async_retval(
            open_stdout=False, open_stderr=False
        )
        chan.status_event.attach_mock(mock.Mock(return_value=True), "is_set")
        ...
        # noinspection PyTypeChecker
        result = ssh.execute(command=command, verbose=False, open_stdout=False, open_stderr=False)

        self.assertEqual(result, exp_result)
        execute_async.assert_called_once_with(command, verbose=False, open_stdout=False, open_stderr=False)
        message = self.gen_cmd_result_log_message(result)
        log = logger(ssh.__class__.__name__).getChild("{host}:{port}".format(host=host, port=port)).log
        log.assert_has_calls([mock.call(level=logging.DEBUG, msg=message)])

    @mock.patch("time.sleep", autospec=True)
    @mock.patch("exec_helpers.ssh_client.SSHClient.execute_async")
    def test_024_execute_timeout(self, execute_async, sleep, client, policy, logger):
        (chan, _stdin, exp_result, stderr, stdout) = self.get_patched_execute_async_retval()
        is_set = mock.Mock(return_value=True)
        chan.status_event.attach_mock(is_set, "is_set")
        ...
        message = self.gen_cmd_result_log_message(result)
        log = logger(ssh.__class__.__name__).getChild("{host}:{port}".format(host=host, port=port)).log
        self.assertIn(mock.call(level=logging.DEBUG, msg=message), log.mock_calls)

    @mock.patch("time.sleep", autospec=True)
    @mock.patch("exec_helpers.ssh_client.SSHClient.execute_async")
    def test_025_execute_timeout_fail(self, execute_async, sleep, client, policy, logger):
        (chan, _stdin, _, stderr, stdout) = self.get_patched_execute_async_retval()
    ...
    @mock.patch("exec_helpers.ssh_client.SSHClient.execute_async")
    def test_026_execute_mask_command(self, execute_async, client, policy, logger):
        cmd = "USE='secret=secret_pass' do task"
        log_mask_re = r"secret\s*=\s*([A-Z-a-z0-9_\-]+)"
        masked_cmd = "USE='secret=<*masked*>' do task"

        (chan, _stdin, exp_result, stderr, stdout) = self.get_patched_execute_async_retval(cmd_log=masked_cmd)
        is_set = mock.Mock(return_value=True)
        chan.status_event.attach_mock(is_set, "is_set")
        ...
        # noinspection PyTypeChecker
        result = ssh.execute(command=cmd, verbose=False, log_mask_re=log_mask_re)

        self.assertEqual(result, exp_result)
        execute_async.assert_called_once_with(cmd, log_mask_re=log_mask_re, verbose=False)
        chan.assert_has_calls((mock.call.status_event.is_set(),))
        message = self.gen_cmd_result_log_message(result)
        log = logger(ssh.__class__.__name__).getChild("{host}:{port}".format(host=host, port=port)).log
        log.assert_has_calls(
    ...
    def test_027_execute_together(self, execute_async, client, policy, logger):
        ...
        # noinspection PyTypeChecker
        results = exec_helpers.SSHClient.execute_together(remotes=remotes, command=command)
        ...
        # noinspection PyTypeChecker
        exec_helpers.SSHClient.execute_together(remotes=remotes, command=command, expected=[1], raise_on_err=False)

        with self.assertRaises(exec_helpers.ParallelCallProcessError):
            # noinspection PyTypeChecker
            exec_helpers.SSHClient.execute_together(remotes=remotes, command=command, expected=[1])

    @mock.patch("exec_helpers.ssh_client.SSHClient.execute_async")
    def test_028_execute_together_exceptions(
        self, execute_async: mock.Mock, client: mock.Mock, policy: mock.Mock, logger: mock.Mock
    ) -> None:
        ...
        self.assertEqual(list(sorted(exc.exceptions)), [(host, port), (host2, port)])
    ...
    def test_029_check_call(self, execute, client, policy, logger):
        exit_code = 0
        return_value = exec_result.ExecResult(cmd=command, stdout=stdout_list, stderr=stderr_list, exit_code=exit_code)
    ...
    def test_030_check_call_expected(self, execute, client, policy, logger):
        ...
        # noinspection PyTypeChecker
        result = ssh.check_call(command=command, verbose=verbose, timeout=None, expected=[0, 75])
        execute.assert_called_once_with(command, verbose, None)
        self.assertEqual(result, return_value)
        ...
        with self.assertRaises(exec_helpers.CalledProcessError):
            # noinspection PyTypeChecker
            ssh.check_call(command=command, verbose=verbose, timeout=None, expected=[0, 75])
        execute.assert_called_once_with(command, verbose, None)

    @mock.patch("exec_helpers.ssh_client.SSHClient.check_call")
    def test_031_check_stderr(self, check_call, client, policy, logger):
        return_value = exec_result.ExecResult(cmd=command, stdout=stdout_list, stderr=[], exit_code=0)
        ...
        # noinspection PyTypeChecker
        result = ssh.check_stderr(command=command, verbose=verbose, timeout=None, raise_on_err=raise_on_err)
        check_call.assert_called_once_with(command, verbose, timeout=None, error_info=None, raise_on_err=raise_on_err)
        self.assertEqual(result, return_value)

        return_value = exec_result.ExecResult(cmd=command, stdout=stdout_list, stderr=stderr_list, exit_code=0)

        check_call.reset_mock()
        check_call.return_value = return_value
        with self.assertRaises(exec_helpers.CalledProcessError):
            # noinspection PyTypeChecker
            ssh.check_stderr(command=command, verbose=verbose, timeout=None, raise_on_err=raise_on_err)
        check_call.assert_called_once_with(command, verbose, timeout=None, error_info=None, raise_on_err=raise_on_err)
...
class TestExecuteThrowHost(unittest.TestCase):
    ...
    def prepare_execute_through_host(transp, client, exit_code):
        ...
        open_channel = mock.Mock(return_value=intermediate_channel, name="open_channel")
        intermediate_transport = mock.Mock(name="intermediate_transport", spec="paramiko.transport.Transport")
        intermediate_transport.attach_mock(open_channel, "open_channel")
        get_transport = mock.Mock(return_value=intermediate_transport, name="get_transport")
        ...
        channel = mock.Mock()
        channel.attach_mock(mock.Mock(return_value=FakeStream(b" \n", b"2\n", b"3\n", b" \n")), "makefile")
        channel.attach_mock(mock.Mock(return_value=FakeStream(b" \n", b"0\n", b"1\n", b" \n")), "makefile_stderr")
        ...
        return (open_session, transport, channel, get_transport, open_channel, intermediate_channel)
    ...
    def test_01_execute_through_host_no_creds(self, transp, client, policy, logger):
        ...
        # noinspection PyTypeChecker
        return_value = exec_result.ExecResult(cmd=command, stderr=stderr_list, stdout=stdout_list, exit_code=exit_code)
        ...
        ) = self.prepare_execute_through_host(transp=transp, client=client, exit_code=exit_code)
        ...
        transport.assert_has_calls(
            (mock.call.connect(username=username, password=password, pkey=None), mock.call.open_session())
        )
    ...
    def test_02_execute_through_host_auth(self, transp, client, policy, logger):
        ...
        (
            ...
            open_channel,
            intermediate_channel,
        ) = self.prepare_execute_through_host(transp, client, exit_code=exit_code)
        ...
        ssh = exec_helpers.SSHClient(
            ...
            auth=exec_helpers.SSHAuth(
                username=username, password=password, key_filename="~/fake_key", passphrase="fake_passphrase"
            ),
        )

        # noinspection PyTypeChecker
        result = ssh.execute_through_host(
            target, command, auth=exec_helpers.SSHAuth(username=_login, password=_password)
        )
        self.assertEqual(result, return_value)
        get_transport.assert_called_once()
        open_channel.assert_called_once()
        ...
        open_session.assert_called_once()
        transport.assert_has_calls(
            (mock.call.connect(username=_login, password=_password, pkey=None), mock.call.open_session())
        )
        channel.assert_has_calls(
            (
                mock.call.makefile("rb"),
    ...
    def test_03_execute_through_host_get_pty(self, transp, client, policy, logger):
        target = "127.0.0.2"
        exit_code = 0
