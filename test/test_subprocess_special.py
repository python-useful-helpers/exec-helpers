...
configs = {
    ...
    "TimeoutError": dict(
        wait=(None,), poll=(None,), stdout=(), expect_exc=exec_helpers.ExecHelperTimeoutError
    ),
    "TimeoutError_closed": dict(
        wait=(None,), poll=(None, 0), stdout=(b" \n", b"2\n", b"3\n", b" \n"), kill=(OSError(),)
    ),
    "TimeoutError_no_kill": dict(
        wait=(None,), poll=(None, None), stdout=(), kill=(OSError(),), expect_exc=OSError
    ),
    ...
    "stdin_error": dict(poll=(0xDEADBEEF,), stdout=(), stdin="Failed", write=OSError(), expect_exc=OSError),
    "stdin_close_closed": dict(stdout=(b" \n", b"2\n", b"3\n", b" \n"), stdin="Stdin", stdin_close=eshutdown_exc),
    "stdin_close_fail": dict(poll=(0xDEADBEEF,), stdout=(), stdin="Failed", stdin_close=OSError(), expect_exc=OSError),
    "mask_global": dict(
...
def exec_result(run_parameters):
    ...
    return exec_helpers.ExecResult(
        ...
        stdout=stdout_res,
        stderr=(),
        exit_code=0 if 0 in run_parameters.get("poll", [0]) else exec_helpers.ExitCodes.EX_INVALID,
    )
...
def create_subprocess_shell(mocker, run_parameters):
    ...
    def create_mock(
        ...
        stdin=None,  # type: typing.Optional[typing.Union[str, bytes, bytearray]]
        **kwargs
    ):
        """Parametrized code."""
        proc = mock.Mock()
        ...
        proc.attach_mock(mock.Mock(side_effect=run_parameters.get("wait", (0,))), "wait")
...
def test_special_cases(create_subprocess_shell, exec_result, logger, run_parameters):
