...
class TestSubprocessRunner(unittest.TestCase):
    ...
    def test_001_call(self, popen: mock.MagicMock, logger: mock.MagicMock, *args) -> None:
        ...
        self.assertEqual(logger.mock_calls[0], mock.call.log(level=logging.DEBUG, msg=command_log))
        self.assertEqual(
    ...
    def test_002_call_verbose(self, popen: mock.MagicMock, logger: mock.MagicMock, *args) -> None:
        ...
        self.assertEqual(logger.mock_calls[0], mock.call.log(level=logging.INFO, msg=command_log))
        self.assertEqual(
    ...
    def test_003_context_manager(self, popen: mock.MagicMock, *args) -> None:
        ...
        with mock.patch("threading.RLock", autospec=True):
            with exec_helpers.Subprocess() as runner:
                ...
                self.assertEqual(mock.call.acquire(), runner.lock.mock_calls[0])
                result = runner.execute(command)
                self.assertEqual(result, exp_result)
    ...
    def test_005_execute_no_stdout(self, popen: mock.MagicMock, logger: mock.MagicMock, *args):
        ...
        logger.assert_has_calls(
            ...
            + [mock.call.log(level=logging.DEBUG, msg=str(x.rstrip().decode("utf-8"))) for x in stderr_list]
    ...
    def test_006_execute_no_stderr(self, popen: mock.MagicMock, logger: mock.MagicMock, *args):
        ...
        logger.assert_has_calls(
            [mock.call.log(level=logging.DEBUG, msg=command_log)]
            + [mock.call.log(level=logging.DEBUG, msg=str(x.rstrip().decode("utf-8"))) for x in stdout_list]
            + [mock.call.log(level=logging.DEBUG, msg=self.gen_cmd_result_log_message(result))]
        )
        self.assertIn(mock.call.wait(timeout=default_timeout), popen_obj.mock_calls)
...
class TestSubprocessRunnerHelpers(unittest.TestCase):
    def test_001_check_call(self, execute, *args):
        ...
        # noinspection PyTypeChecker
        result = runner.check_call(command=command, verbose=verbose, timeout=None)
    ...
    def test_002_check_call_expected(self, execute, *args):
        ...
        # noinspection PyTypeChecker
        result = runner.check_call(command=command, verbose=verbose, timeout=None, expected=[0, 75])
        execute.assert_called_once_with(command, verbose, None)
        self.assertEqual(result, return_value)
        ...
        with self.assertRaises(exec_helpers.CalledProcessError):
            # noinspection PyTypeChecker
            runner.check_call(command=command, verbose=verbose, timeout=None, expected=[0, 75])
        execute.assert_called_once_with(command, verbose, None)
    ...
    def test_003_check_stderr(self, check_call, *args):
        ...
        # noinspection PyTypeChecker
        result = runner.check_stderr(command=command, verbose=verbose, timeout=None, raise_on_err=raise_on_err)
        check_call.assert_called_once_with(command, verbose, timeout=None, error_info=None, raise_on_err=raise_on_err)
        self.assertEqual(result, return_value)

        return_value = exec_helpers.ExecResult(cmd=command, stdout=stdout_list, stderr=stdout_list, exit_code=0)

        check_call.reset_mock()
        check_call.return_value = return_value
        with self.assertRaises(exec_helpers.CalledProcessError):
            # noinspection PyTypeChecker
            runner.check_stderr(command=command, verbose=verbose, timeout=None, raise_on_err=raise_on_err)
        check_call.assert_called_once_with(command, verbose, timeout=None, error_info=None, raise_on_err=raise_on_err)
