...
def test_001_mask_command(ssh, get_logger):
...
def test_002_mask_command_global(ssh, get_logger):
    cmd = "USE='secret=secret_pass' do task"
    log_mask_re = r"secret\s*=\s*([A-Z-a-z0-9_\-]+)"
    masked_cmd = "USE='secret=<*masked*>' do task"
    cmd_log = "Executing command:\n{!r}\n".format(masked_cmd)
...
def test_004_execute_timeout(ssh):
    """We allow timeout and not crush on it if fit."""
    ssh.execute(command, timeout=0.01)


def test_005_execute_timeout_fail(ssh, ssh_transport_channel, exec_result):
    """We allow timeout and not crush on it if fit."""
...
def test_006_execute_together_exceptions(ssh, ssh2, mocker):
