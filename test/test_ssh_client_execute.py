...
def execute_async(mocker, run_parameters):
    def get_patched_execute_async_retval(ec=0, stdout=(), stderr=(), open_stdout=True, open_stderr=True, **kwargs):
...
def test_002_execute(ssh, ssh_transport_channel, exec_result, run_parameters):
    kwargs = {}
    if "get_pty" in run_parameters:
...
def test_003_context_manager(ssh, exec_result, run_parameters, mocker):
    kwargs = {}
    if "get_pty" in run_parameters:
        kwargs["get_pty"] = run_parameters["get_pty"]
    if "width" in run_parameters:
...
def test_004_check_call(ssh, exec_result, get_logger, mocker):
...
def test_005_check_call_no_raise(ssh, exec_result, get_logger, mocker):
    mocker.patch("exec_helpers.ssh_client.SSHClient.execute", return_value=exec_result)
    ssh_logger = get_logger(exec_helpers.SSHClient.__name__)
    log = ssh_logger.getChild("{host}:{port}".format(host=host, port=port))
...
def test_006_check_call_expect(ssh, exec_result, mocker):
    mocker.patch("exec_helpers.ssh_client.SSHClient.execute", return_value=exec_result)
    assert ssh.check_call(command, stdin=exec_result.stdin, expected=[exec_result.exit_code]) == exec_result


def test_007_check_stderr(ssh, exec_result, get_logger, mocker):
...
def test_008_check_stderr_no_raise(ssh, exec_result, mocker):
    mocker.patch("exec_helpers.ssh_client.SSHClient.check_call", return_value=exec_result)
    assert ssh.check_stderr(
